...
library(igraph)
 ...
 dim(ownPhages_sub)
 ownPhages_sub <- ownPhages_sub %>% filter(!from %in% c("VIRSorter_Ga0401713_04","VIRSorter_Ga0401710_02") )%>% filter(!to %in% c("VIRSorter_Ga0401713_04","VIRSorter_Ga0401710_02"))
  dim(ownPhages_sub_tmp)
...
dev.off()
 ...
 htmlwidgets::saveWidget(ggp, "~/Desktop/Projects/Bifidos/06_vContact/OurPhages_and_GermansPhages/allPhages_vContact/networkPlot_subset_corrected.html")
 # View(nExtended)
...
```
...or
Here, we extract the CRISPR spacers from the CRISPRcasfinder output. 
...
We can look if the extracted CRISPR spacers are unique or shared amongst the different genomes. 
...
grep ">" /home/vincent/Desktop/Projects/Bifidos/07_CRISPR_cas/spacers/all_spacers.fasta  |sed 's/>//g' |awk -F "[_-]" '{OFS="\t"}{print $0, $1,$1"_"$2}' > /home/vincent/Desktop/Projects/Bifidos/07_CRISPR_cas/spacers/all_spacers.txt

```

```{r}
library(readr)
all_spacers <- read_delim("~/Desktop/Projects/Bifidos/07_CRISPR_cas/spacers/all_spacers.txt", "\t", escape_double = FALSE, col_names = c("spacerName","Genome","arrayName"), trim_ws = TRUE)

spacerDescrition <- ggplot(all_spacers,aes(x=Genome,fill=arrayName))+geom_bar()+theme_classic()


 png("~/Desktop/Projects/Bifidos/07_CRISPR_cas/spacers/SpacerDescription.png", width = 3000, height = 1500,res=300)
...
##### blast spacers
We know iteratively blast against the different blast DB. 
...
cat /data/Project/Bifidos/Germans_phages/all_bifiod/all_bifido_phages.fasta \
/data/Project/Bifidos/Germans_phages/all_bifiod/own_bifido_phages.txt > /data/Project/Bifidos/Germans_phages/all_bifiod/own_bifido_phages_ncbi.txt
...
transfer local
```{bash}
/data/Project/Bifidos/Germans_phages/all_bifiod/own_bifido_phages_ncbi.txt

scp vincent@130.223.51.116:/data/Project/Bifidos/Germans_phages/all_bifiod/own_bifido_phages_ncbi.txt /home/vincent/Desktop/Projects/Bifidos
...
library(ggplot2)
library(tidyverse)
virus_genomes_mapping <- read_csv("~/Desktop/Projects/Bifidos/07_CRISPR_cas/spacers/blast/viruses/BMTU5RA801R-Alignment-HitTable.csv", col_names = c("query acc.ver", "subject acc.ver", "% identity", "alignment length", "mismatches", "gap opens", "q. start", "q. end", "s. start", "s. end", "evalue", "bit score"))

ggplot(virus_genomes_mapping, aes(x=`% identity`))+geom_histogram()+theme_classic()
unique(virus_genomes_mapping_02$`query acc.ver`) %>% length()
virus_genomes_mapping_02 <- virus_genomes_mapping %>% filter(`% identity`==100)
unique(virus_genomes_mapping_02$`query acc.ver`) %>% length()

# virus_genomes_mapping_02$evalue
ggplot(virus_genomes_mapping_02, aes(x=`alignment length`))+geom_histogram()+theme_classic()
ggplot(virus_genomes_mapping_02, aes(x=evalue))+geom_histogram()+theme_classic()
virus_genomes_mapping_03 <- virus_genomes_mapping_02 %>% filter(evalue<0.1)
unique(virus_genomes_mapping_03$`query acc.ver`) %>% length()

```


output of own DB and loca phages
```{r}
...
library(ggplot2)
library(tidyverse)
# virus_genomes_mapping <- read_csv("~/Desktop/Projects/Bifidos/07_CRISPR_cas/spacers/blast/ownViruses/BMVNXDJ9114-Alignment-HitTable.csv")

virus_genomes_mapping <- read_csv("~/Desktop/Projects/Bifidos/07_CRISPR_cas/spacers/blast/ownViruses/BMVNXDJ9114-Alignment-HitTable.csv", col_names = c("query acc.ver", "subject acc.ver", "% identity", "alignment length", "mismatches", "gap opens", "q. start", "q. end", "s. start", "s. end", "evalue", "bit score"))


#tidyr
virus_genomes_mapping_02 <- virus_genomes_mapping %>% filter(`% identity`==100)
#basic
# virus_genomes_mapping_02 <- virus_genomes_mapping[which(virus_genomes_mapping$`% identity`==100),]
...
##only own and germans phages

virus_genomes_mapping_04 <- virus_genomes_mapping_03[grep("_",virus_genomes_mapping_03$`subject acc.ver`),2] 

# virus_genomes_mapping_germans <- virus_genomes_mapping_04[grep("GR7_",virus_genomes_mapping_04$`subject acc.ver`),2]
# virus_genomes_mapping_germans_02 <- virus_genomes_mapping_04[grep("LD7_",virus_genomes_mapping_04$`subject acc.ver`),2]
# virus_genomes_mapping_assembled <- virus_genomes_mapping_04[grep("Ga0",virus_genomes_mapping_04$`subject acc.ver`),2]
tmp1 <- str_split_fixed(virus_genomes_mapping_04$`subject acc.ver`, "_", 6)[,1] 
tmp2 <- str_split_fixed(virus_genomes_mapping_04$`subject acc.ver`, "_", 6)[,2] 
query <- paste(tmp1,"_",tmp2,sep="")


for (rowzzz in query) {
  print(rowzzz)
  n$vertex.names[grep(rowzzz,n$vertex.names)] %>% as.character()
 
}


library(readr)
own_cluster_sub_new <- read_csv("~/Desktop/Projects/Bifidos/06_vContact/OurPhages_and_GermansPhages/allPhages_vContact/Streptococcus_phage_betwork_connections.txt")
 Sterm_net <- graph_from_data_frame(own_cluster_sub_new,direct=F)
...
for (rowzzz in query) {
  print(rowzzz)
  # n$vertex.names[grep(rowzzz,n$vertex.names)] %>% as.character()
  n$color[grep(rowzzz,n$vertex.names)] <- "matched genomes"

}
 n$color = factor(n$color, levels=c("matched genomes","Virsorter Contigs","Germans contigs","Viral DB"))

  
   # n$vertex.names[grep("VIRSorter",n$vertex.names)]
   
 # n$color[grep("VC_",n$vertex.names)] <- "Germans contigs"
 # n$color[grep("VIRSorter",n$vertex.names)] <- "Virsorter Contigs"
 # n$color = factor(n$color, levels=c("Virsorter Contigs","Germans contigs","Viral DB"))
 colorzzz <- c("pink","red","blue","grey50")
 size <- c("6","4","2","1.5")
 ##------------------
 ##extend
 ##------------------
...
colnames(nExtended) <- revalue(colnames(nExtended), c("Adj P-value"="Adj_P_value","Genera in VC"="Genera_in_VC","Topology Confidence Score"="Topology_Confidence_Score","Genus Confidence Score"="Genus_Confidence_Score"))
 ...
 png("~/Desktop/Projects/Bifidos/06_vContact/OurPhages_and_GermansPhages/allPhages_vContact/networkPlot_matched.png", width = 3000, height = 3000,res=300)
...
dev.off()
 ...
 library(plotly)
 ggp <- ggplotly(networkplot)
 htmlwidgets::saveWidget(ggp, "~/Desktop/Projects/Bifidos/06_vContact/OurPhages_and_GermansPhages/allPhages_vContact/networkPlot_matched.html")
 # View(nExtended)
 # unique(nExtended$vertex.names)
 ggp
